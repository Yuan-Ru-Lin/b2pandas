...
class TestVector3D(unittest.TestCase):
    ...
    def test__mul__(self):
        ...
        np.testing.assert_allclose(self.x * self.x, self.df[0]**2 + self.df[1]**2 + self.df[2]**2)
        
        # Verify the multiplication in Vector3D * Number (= Vector3D)
        self.assertTrue(self.x * self.y == Vector3D(self.df[0] * self.df[3], self.df[1] * self.df[3], self.df[2]* self.df[3]))
        self.assertTrue(self.x.__mul__(self.y) == Vector3D(self.df[0] * self.df[3], self.df[1] * self.df[3], self.df[2]* self.df[3]))
